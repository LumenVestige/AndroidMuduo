...
*/
    ...
    const char *strerror_tl(int savedErrno) {
        int rc = strerror_r(savedErrno, t_errnobuf, sizeof t_errnobuf);
        if (rc == 0) {
            return t_errnobuf;
        } else {
...
// helper class for known string length at compile time
    ...
    void defaultOutput(const char *msg, int len) {
        ...
        int start = 0;
        while (start < len) {
            int chunkLen = std::min(kMaxLogLen, len - start);
            __android_log_print(priority, "muduo", "%.*s", chunkLen, msg + start);
            start += chunkLen;
        }
#else
